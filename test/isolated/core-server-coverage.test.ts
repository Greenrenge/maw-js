...
mock.module(import.meta.resolve("../../src/core/engine-plugin-registry"), () => ({
  dispatchEnginePluginEvent: async (event: unknown) => { enginePluginDispatches.push(event); },
...
mock.module(import.meta.resolve("../../src/plugins/index"), () => ({
  ...
  watchUserPlugins: (dir: string) => { pluginWatchDirs.push(dir); },
...
mock.module(import.meta.resolve("../../src/lib/peers/store"), () => ({ loadPeers: () => ({ peers: { remote: { node: "remote" } } }) }));
...
const { startServer, views } = await import("../../src/core/server.ts?core-server-coverage");
